...
| Trạng thái thực thi | Đã thực thi qua đọc UI Web Admin và gọi API kiểu Postman/cURL ngày 2026-07-09. Evidence: `artifacts/test-results/FR18_order_management_admin_api_evidence.txt`. |
...
| --- | --- | --- | --- | --- | --- | --- |
| EP-FR18-STATUS-001 | Admin xác nhận đơn `pending`. | Tiền điều kiện: đơn `O_pending` đang có `status=pending`. Web: chọn thao tác xác nhận. API: `PUT /api/admin/orders/O_pending/status` với admin token và body `{"status":"confirmed"}`. | EC01, EC05, EC07, EC10, EC21 | Web: đơn chuyển sang trạng thái `confirmed` và danh sách/chi tiết phản ánh trạng thái mới. API: contract chưa nêu status/body; request hợp lệ thành công theo SUT và lần đọc lại đơn/danh sách thấy `status=confirmed`. | API trả `200 {"message":"Order status updated"}`; đọc lại `O1` thấy `status="confirmed"`. UI có nút `Xác nhận` cho `pending` và gọi đúng target. Evidence: `STATUS-001...`, `Read O1 after STATUS-001`. | Đạt |
| EP-FR18-STATUS-002 | Admin hủy đơn `pending`. | `O_pending` có `status=pending`; gửi target `canceled`. | EC01, EC05, EC07, EC11, EC21 | Web: đơn chuyển thành `canceled` và không còn hành động chuyển tiếp hợp lệ. API: contract chưa nêu status/body; request hợp lệ thành công theo SUT và lần đọc lại thấy `status=canceled`. | API trả `200`; đọc lại `O2` thấy `status="canceled"`. Tuy nhiên UI code vẫn hiển thị nút `Đánh dấu Đã giao` cho `canceled`, trái final-state kỳ vọng ở bước sau. Evidence: `STATUS-002...`, `Read O2 after STATUS-002`. | Đạt một phần |
| EP-FR18-STATUS-003 | Admin chuyển `confirmed` sang `shipping`. | `O_confirmed` có `status=confirmed`; gửi target `shipping`. | EC01, EC05, EC07, EC12, EC21 | Web: đơn chuyển thành `shipping` và trạng thái mới quan sát được khi xem lại. API: contract chưa nêu status/body; request hợp lệ thành công theo SUT và lần đọc lại thấy `status=shipping`. | API trả `200`; đọc lại `O1` thấy `status="shipping"`. UI có nút `Giao hàng` cho `confirmed`. Evidence: `STATUS-003...`, `Read O1 after STATUS-003`. | Đạt |
| EP-FR18-STATUS-004 | Admin hủy đơn `confirmed`. | `O_confirmed` có `status=confirmed`; gửi target `canceled`. | EC01, EC05, EC07, EC13, EC21 | Web: đơn chuyển thành `canceled`; sau đó đơn là final state. API: contract chưa nêu status/body; request hợp lệ thành công theo SUT và lần đọc lại thấy `status=canceled`. | Trên order sạch `O8`, setup `pending -> confirmed` trả `200`, sau đó `confirmed -> canceled` trả `200`; đọc lại thấy `status="canceled"`. Evidence: `STATUS-004 setup O8...`, `STATUS-004 clean...`, `Read O8...`. | Đạt |
| EP-FR18-STATUS-005 | Admin hoàn tất đơn `shipping`. | `O_shipping` có `status=shipping`; gửi target `delivered`. | EC01, EC05, EC07, EC14, EC21 | Web: đơn chuyển thành `delivered`; sau đó đơn là final state và không được chuyển tiếp. API: contract chưa nêu status/body; request hợp lệ thành công theo SUT và lần đọc lại thấy `status=delivered`. | API trả `200`; đọc lại `O1` thấy `status="delivered"`. UI có nút `Hoàn thành` cho `shipping`. Evidence: `STATUS-005...`, `Read O1 after STATUS-005`. | Đạt |
| EP-FR18-STATUS-006 | Từ chối khi thiếu token. | Gọi `PUT /api/admin/orders/O_pending/status` không gửi `Authorization`, body `{"status":"confirmed"}`. | EC02, EC05, EC07, EC22 | Web: không cho thao tác khi chưa đăng nhập admin. API: trả lỗi xác thực `4xx` theo contract thực tế; `O_pending` vẫn là `pending`. | API trả `401 {"error":"Unauthorized"}`. UI khi chưa có token chỉ hiển thị form login nên không có nút thao tác. Evidence: `STATUS-006 missing token O3`. | Đạt |
| EP-FR18-STATUS-007 | Từ chối khi token không hợp lệ. | Gọi endpoint bằng `Authorization: Bearer invalid.token`, body hợp lệ. | EC03, EC05, EC07, EC22 | API: trả lỗi xác thực `4xx`; trạng thái đơn không đổi. | API trả `403 {"error":"Forbidden"}`. Evidence: `STATUS-007 invalid token O3`. | Đạt |
| EP-FR18-STATUS-008 | Từ chối user thường cập nhật trạng thái admin. | Dùng token user thường gọi `PUT /api/admin/orders/O_pending/status` với body `{"status":"confirmed"}`. | EC04, EC05, EC07, EC22 | Web: user thường không thao tác được chức năng cập nhật trạng thái admin. API: trả lỗi phân quyền `4xx` theo contract thực tế; trạng thái đơn không đổi. | API trả `200 {"message":"Order status updated"}` cho token user thường; đọc lại `O9` thấy `pending` đã đổi thành `confirmed`. UI login chặn user thường khi đăng nhập qua form, nhưng backend endpoint vẫn không kiểm role. Evidence: `STATUS-008 clean normal user token O9`, `Read O9...`. | Không đạt |
| EP-FR18-STATUS-009 | Từ chối đơn không tồn tại. | Admin token hợp lệ; gọi `PUT /api/admin/orders/O_missing/status` với body `{"status":"confirmed"}`. | EC01, EC06, EC07, EC22 | API/Web không được tạo mới hoặc cập nhật đơn không tồn tại; trả lỗi phù hợp theo contract thực tế. | API trả `404 {"error":"Order not found"}` cho `id=999999`. Evidence: `STATUS-009 missing order`. | Đạt |
| EP-FR18-STATUS-010 | Từ chối body thiếu `status`. | `O_pending` tồn tại; admin token hợp lệ; body `{}`. | EC01, EC05, EC08, EC22 | Web: không gửi thao tác thiếu trạng thái hoặc hiển thị lỗi phù hợp nếu UI cho phép. API: trả lỗi theo contract thực tế vì thiếu trạng thái đích; `O_pending` vẫn là `pending`. | API trả `400 {"error":"Invalid state transition from confirmed to undefined"}`. UI không có thao tác gửi thiếu status trong luồng bình thường. Evidence: `STATUS-010 missing status body O3`. | Đạt |
| EP-FR18-STATUS-011 | Từ chối `status` ngoài tập hợp lệ. | `O_pending` tồn tại; body `{"status":"returned"}`. | EC01, EC05, EC09, EC22 | Web: không có lựa chọn trạng thái ngoài tập hợp lệ hoặc hiển thị lỗi nếu thao tác xảy ra. API: trả lỗi theo contract thực tế vì `status` không thuộc tập API spec; trạng thái đơn không đổi. | API trả `400 {"error":"Invalid state transition from confirmed to returned"}`. UI không có nút target `returned`. Evidence: `STATUS-011 invalid status O3`. | Đạt |
| EP-FR18-STATUS-012 | Từ chối bỏ qua bước `pending -> shipping`. | `O_pending` có `status=pending`; gửi target `shipping`. | EC01, EC05, EC07, EC15, EC22 | Web: không cho thao tác bỏ qua bước hoặc hiển thị lỗi chuyển đổi không hợp lệ. API: trả lỗi chuyển đổi không hợp lệ theo contract thực tế; `O_pending` vẫn là `pending`. | Trên order sạch `O6`, API trả `400 {"error":"Invalid state transition from pending to shipping"}`; đọc lại vẫn `status="pending"`. UI không có nút `Giao hàng` khi order đang `pending`. Evidence: `STATUS-012 clean pending to shipping O6`, `Read O6...`. | Đạt |
| EP-FR18-STATUS-013 | Từ chối bỏ qua bước `confirmed -> delivered`. | `O_confirmed` có `status=confirmed`; gửi target `delivered`. | EC01, EC05, EC07, EC15, EC22 | Web: không cho thao tác bỏ qua `shipping` hoặc hiển thị lỗi chuyển đổi không hợp lệ. API: trả lỗi chuyển đổi không hợp lệ theo contract thực tế; `O_confirmed` vẫn là `confirmed`. | API trả `400 {"error":"Invalid state transition from confirmed to delivered"}` sau khi `O4` được setup `confirmed`. UI không có nút `Hoàn thành` khi order đang `confirmed`. Evidence: `STATUS-013 confirmed to delivered O4`. | Đạt |
| EP-FR18-STATUS-014 | Từ chối đi lùi trạng thái. | `O_confirmed` có `status=confirmed`; gửi target `pending`. | EC01, EC05, EC07, EC16, EC22 | Web: không cho thao tác đi lùi hoặc hiển thị lỗi chuyển đổi không hợp lệ. API: trả lỗi theo contract thực tế; trạng thái không đổi. | API trả `400 {"error":"Invalid state transition from confirmed to pending"}`. UI không có nút quay về `pending`. Evidence: `STATUS-014 confirmed to pending O4`. | Đạt |
| EP-FR18-STATUS-015 | Từ chối cập nhật về cùng trạng thái. | `O_pending` có `status=pending`; gửi target `pending`. | EC01, EC05, EC07, EC17, EC22 | Web: không được coi là chuyển đổi hợp lệ theo sơ đồ; trạng thái vẫn `pending`. API: kỳ vọng trả lỗi/no-op không đổi trạng thái; nếu API trả thành công, cần người học review theo oracle bài vì contract chưa nêu trường hợp này. | Trên order sạch `O7`, API trả `400 {"error":"Invalid state transition from pending to pending"}`; đọc lại vẫn `status="pending"`. Evidence: `STATUS-015 clean pending to pending O7`, `Read O7...`. | Đạt |
| EP-FR18-STATUS-016 | Rà soát trường hợp mơ hồ `shipping -> canceled`. | `O_shipping` có `status=shipping`; gửi target `canceled`. | EC01, EC05, EC07, EC18, EC22 | Theo sơ đồ FR-10, Web/API phải từ chối và giữ `shipping`; ghi nhận actual để review vì README có câu mơ hồ về quyền admin khi đơn đang `shipping`. | API trả `400 {"error":"Invalid state transition from shipping to canceled"}`; đọc lại `O4` vẫn `status="shipping"`. UI chỉ có nút `Hoàn thành` cho `shipping`, không có nút `Hủy`. Evidence: `STATUS-016...`, `Read O4...`. | Đạt theo oracle sơ đồ |
| EP-FR18-STATUS-017 | Từ chối chuyển từ final state `delivered`. | `O_delivered` có `status=delivered`; gửi target `shipping` hoặc `canceled`. | EC01, EC05, EC07, EC19, EC22 | Web: không cho chuyển tiếp từ `delivered` hoặc hiển thị lỗi phù hợp. API: trả lỗi theo contract thực tế; `O_delivered` vẫn là `delivered`. | API trả `400 {"error":"Invalid state transition from delivered to shipping"}`. UI không hiển thị nút action cho `delivered`. Evidence: `STATUS-017 delivered to shipping O1`. | Đạt |
| EP-FR18-STATUS-018 | Từ chối chuyển từ final state `canceled`. | `O_canceled` có `status=canceled`; gửi target `confirmed`. | EC01, EC05, EC07, EC20, EC22 | Web: không cho chuyển tiếp từ `canceled` hoặc hiển thị lỗi phù hợp. API: trả lỗi theo contract thực tế; `O_canceled` vẫn là `canceled`. | API từ chối `canceled -> confirmed` (`400`), nhưng lại chấp nhận `canceled -> delivered` (`200`) và đọc lại `O2` thấy `status="delivered"`. UI cũng có nút `Đánh dấu Đã giao` khi `status === "canceled"`. Evidence: `STATUS-018 canceled to confirmed O2`, `STATUS canceled to delivered O2 implementation check`, `Read O2 after canceled->delivered`. | Không đạt |
...
| --- | --- | --- | --- | --- | --- | --- |
| BV-FR18-STATUS-001 | State machine | Cạnh đầu hợp lệ | `O_pending`: cập nhật `pending -> confirmed` bằng admin token. | Web: cập nhật thành `confirmed`; đọc lại thấy trạng thái mới. API: request thành công theo SUT và lần đọc lại thấy `status=confirmed`. | API `200`, đọc lại `O1.status="confirmed"`. Evidence: `STATUS-001...`. | Đạt |
| BV-FR18-STATUS-002 | State machine | Bỏ qua cạnh đầu | `O_pending`: cập nhật `pending -> shipping`. | Web: từ chối thao tác hoặc hiển thị lỗi; đọc lại vẫn `pending`. API: trả lỗi theo contract thực tế; đọc lại vẫn `pending`. | API `400`, đọc lại `O6.status="pending"`. Evidence: `STATUS-012 clean pending to shipping O6`. | Đạt |
| BV-FR18-STATUS-003 | State machine | Cạnh giữa hợp lệ | `O_confirmed`: cập nhật `confirmed -> shipping`. | Web: cập nhật thành `shipping`; đọc lại thấy trạng thái mới. API: request thành công theo SUT và lần đọc lại thấy `status=shipping`. | API `200`, đọc lại `O1.status="shipping"`. Evidence: `STATUS-003...`. | Đạt |
| BV-FR18-STATUS-004 | State machine | Bỏ qua cạnh giữa | `O_confirmed`: cập nhật `confirmed -> delivered`. | Web: từ chối thao tác hoặc hiển thị lỗi; đọc lại vẫn `confirmed`. API: trả lỗi theo contract thực tế; đọc lại vẫn `confirmed`. | API `400 {"error":"Invalid state transition from confirmed to delivered"}`. Evidence: `STATUS-013...`. | Đạt |
| BV-FR18-STATUS-005 | State machine | Cạnh trước final `delivered` | `O_shipping`: cập nhật `shipping -> delivered`. | Web: cập nhật thành `delivered`; đọc lại thấy trạng thái mới. API: request thành công theo SUT và lần đọc lại thấy `status=delivered`. | API `200`, đọc lại `O1.status="delivered"`. Evidence: `STATUS-005...`. | Đạt |
| BV-FR18-STATUS-006 | State machine | Sau final `delivered` | `O_delivered`: cập nhật `delivered -> shipping`. | Web: từ chối vì `delivered` là final state; đọc lại vẫn `delivered`. API: trả lỗi theo contract thực tế; đọc lại vẫn `delivered`. | API `400`; UI không có action cho `delivered`. Evidence: `STATUS-017 delivered to shipping O1`. | Đạt |
| BV-FR18-STATUS-007 | State machine | Cạnh vào final `canceled` | `O_pending` hoặc `O_confirmed`: cập nhật sang `canceled`. | Web: cập nhật thành `canceled`; đơn trở thành final state. API: request thành công theo SUT và lần đọc lại thấy `status=canceled`. | API `200` cho `pending -> canceled` và `confirmed -> canceled`; đọc lại thấy `status="canceled"`. Nhưng UI/API sau đó vẫn cho đường `canceled -> delivered`, nên final-state không được bảo toàn. Evidence: `STATUS-002...`, `STATUS-004 clean...`, `STATUS canceled to delivered...`. | Đạt một phần |
| BV-FR18-STATUS-008 | State machine | Sau final `canceled` | `O_canceled`: cập nhật `canceled -> confirmed`. | Web: từ chối vì `canceled` là final state; đọc lại vẫn `canceled`. API: trả lỗi theo contract thực tế; đọc lại vẫn `canceled`. | API từ chối `canceled -> confirmed` nhưng chấp nhận `canceled -> delivered` và UI có nút `Đánh dấu Đã giao` cho `canceled`, nên final-state `canceled` bị vi phạm. Evidence: `STATUS-018...`, `STATUS canceled to delivered...`. | Không đạt |
...
- Lỗi phát hiện khi thực thi: API admin không kiểm `role=admin`, user thường cập nhật được trạng thái đơn hàng qua `/api/admin/orders/:id/status`.
- Lỗi phát hiện khi thực thi: `canceled` không được bảo toàn như final state vì Web Admin có nút `Đánh dấu Đã giao` cho đơn đã hủy và API chấp nhận `canceled -> delivered`.
