...
| --- | --- | --- | --- | --- | --- | --- |
| BV-FR18-LIST-001 | Số lượng đơn hàng | `0` đơn | Admin token hợp lệ; môi trường không có đơn hàng. Web/API lấy danh sách đơn hàng admin. | Web: hiển thị danh sách rỗng/empty state phù hợp. API: trả danh sách rỗng theo contract thực tế; không lỗi hệ thống. | API trả `200 []`; UI không có dữ liệu giả nhưng thiếu empty-state text rõ ràng. Evidence: `LIST-001 initial empty admin orders`. | Đạt một phần |
| BV-FR18-LIST-002 | Số lượng đơn hàng | `1` đơn | Admin token hợp lệ; có đúng một đơn `O_U1`. | Web/API hiển thị/trả đúng một đơn `O_U1`, không nhân đôi hoặc bỏ sót. | Qua code UI và API item `orderId=1`, mỗi order được render bằng một `<tr key={o.id}>`; response không nhân đôi `id=1`. Evidence: `Create O1...`, `Read final admin list`. | Đạt |
| BV-FR18-LIST-003 | Số lượng đơn hàng | `2` đơn | Admin token hợp lệ; có hai đơn `O_U1`, `O_U2`. | Web/API hiển thị/trả cả hai đơn. | API trả nhiều hơn 2 đơn và có đủ `id=1`, `id=2`; không bỏ sót khi danh sách vượt biên `2`. Evidence: `LIST-002/003 admin list after multi-user orders`. | Đạt |
| BV-FR18-LIST-004 | Số lượng user có đơn | `1` user | Có một hoặc nhiều đơn nhưng đều thuộc `U1`. | Web/API hiển thị toàn bộ đơn của `U1`; không lỗi khi chỉ có một user có đơn. | Các đơn của `Test User` (`user_id=2`) xuất hiện đầy đủ cùng `user_name`. UI render theo response, không lỗi ở trường hợp một user có nhiều đơn. Evidence: `LIST-002/003...`. | Đạt |
| BV-FR18-LIST-005 | Số lượng user có đơn | `2` user | Có `O_U1` thuộc `U1` và `O_U2` thuộc `U2`. | Web/API hiển thị/trả đơn của cả `U1` và `U2`; nếu chỉ hiển thị một user thì không đạt FR-18. | API trả cả `Test User` và `FR18 User 2` trong cùng danh sách admin. Evidence: `LIST-002/003 admin list after multi-user orders`. | Đạt |
