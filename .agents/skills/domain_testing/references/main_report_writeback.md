...
Use this reference only after the user has approved the final Domain Testing step for a feature.

## Trigger

Write to `HW02/Main_Report.md` when either condition is true:

- The user explicitly asks to write/update the main report after Step 4.
- The user approves Step 4 and previously requested automatic report writing.

Do not write after Step 1, Step 2, or Step 3 approval.
...
- `### n.2 Domain Testing / EP`

Do not modify BVA, AI Gap Analysis, other features, execution results, bug links, or unrelated report sections unless the user explicitly asks.
...
5. Inside only that feature block, replace only the content from `### n.2 Domain Testing / EP` up to the next sibling heading, usually `### n.3 Boundary Value Analysis`.
6. If the section is missing, insert `### n.2 Domain Testing / EP` before the BVA section in the same feature block.
